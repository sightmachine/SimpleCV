...
class DFTFilter():
	...
	def _boundsFromPercentage(self, floatVal, bound):
		#This is a private method, only used within this class by its defs.
		
		return np.clip(int(floatVal*(bound/2.00)),0,(bound/2))
		#return np.clip(int(floatVal*bound),0,bound)
		
	def createLowPassFilter(self, width,height, xCutoff,yCutoff=None,grayscale=False):
		"""
		**SUMMARY**
		...
		**PARAMETERS**
		* *width* - width of the filter. Must be same as the required image.
		* *height* - height of the filter. Must be same as the required image.
		...
		**SEE ALSO**
		...
		for i in range(0,len(xCutoff)):
			xCutoff[i] = self._boundsFromPercentage(xCutoff[i],width)
			yCutoff[i] = self._boundsFromPercentage(yCutoff[i],height)
		...
		if( grayscale ):
			filter = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)            
			cv.Zero(filter)
			...
			filterB = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
			filterG = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
			filterR = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
			cv.Zero(filterB)
			cv.Zero(filterG)
			cv.Zero(filterR)
			#now make all of the corners black
			...
			filter = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,3)
			cv.Merge(filterB,filterG,filterR,None,filter)

		scvFilt = Image(filter)
		return scvFilt

	def createHighPassFilter(self,width,height, xCutoff,yCutoff=None,grayscale=False):
		"""
		**SUMMARY**
		...
		**SEE ALSO**
		...
		if( grayscale ):
			...
			cv.AddS(filterG,255,filterG) # make everything white
	...
	def createBandPassFilter(self, width, height, xCutoffLow, xCutoffHigh, yCutoffLow=None, yCutoffHigh=None,grayscale=False):
		"""
		**SUMMARY**
		...
		**EXAMPLE**
		...
		>>> img = Image("SimpleCV/sampleimages/RedDog2.jpg")
		>>> df = DFTFilter()
		>>> bpf = df.bandPassFilter(img.width,img.height,[0.2,0.2,0.05],[0.3,0.3,0.2])
		>>> bpf = img.applyDFTFilter(bpf)
		>>> bpf.show()
		>>> bpf.getDFTLogMagnitude().show()
		...
		**SEE ALSO**
		...
		for i in range(0,len(xCutoffLow)):
			xCutoffLow[i] = self._boundsFromPercentage(xCutoffLow[i],width)
			xCutoffHigh[i] = self._boundsFromPercentage(xCutoffHigh[i],width)
			yCutoffHigh[i] = self._boundsFromPercentage(yCutoffHigh[i],height)  
			yCutoffLow[i] = self._boundsFromPercentage(yCutoffLow[i],height)
		...
		if( grayscale ):
			...
			#I need to look CVMERGE/SPLIT... really need to know
			# how much memory we're allocating here
			filterB = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
			filterG = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
			filterR = cv.CreateImage((width,height),cv.IPL_DEPTH_8U,1)
	...
	def createButterworthFilter(self, width, height, dia=400,order=2,highpass=False,grayscale=False):
		"""
		**SUMMARY**
		...
		**EXAMPLE**
		...
		>>> im = Image("lenna")
		>>> df = DFTFilter()
		>>> filter = df.createButterworth(im.width, im.height, dia=400,order=2,highpass=True,grayscale=False)
	    >>> img = im.applyDFTFilter(filter)
		Output image: http://i.imgur.com/5LS3e.png

		>>> filter = df.createButterworth(im.width, im.height, dia=400,order=2,highpass=False,grayscale=False)
		>>> img = im.applyDFTFilter(filter)
		
		Output img: http://i.imgur.com/QlCAY.png

		>>> im = Image("grayscale_lenn.png") #take image from here: http://i.imgur.com/O0gZn.png
		>>> filter = df.createButterworth(im.width, im.height, dia=400,order=2,highpass=True,grayscale=True)
		>>> img = im.applyDFTFilter(filter)
		
		Output img: http://i.imgur.com/BYYnp.png

		>>> filter = df.createButterworth(im.width, im.height, dia=400,order=2,highpass=False,grayscale=True)
		>>> img = im.applyDFTFilter(filter)
	...
	def createGaussianFilter(self, width, height, dia=400, highpass=False, grayscale=False):
		"""
		**SUMMARY**
		...
		**EXAMPLE**
		...
		>>> im = Image("lenna")
		>>> df = DFTFilter()
		>>> filter = df.createGaussianFilter(im.width, im.height, dia=400,highpass=True,grayscale=False)
		>>> img = im.applyDFTFilter(filter)

		Output image: http://i.imgur.com/DttJv.png

		>>> filter = df.createGaussianFilter(im.width, im.height, dia=400,highpass=False,grayscale=False)
		>>> img = im.applyDFTFilter(filter)
		
		Output img: http://i.imgur.com/PWn4o.png

		>>> im = Image("grayscale_lenn.png") #take image from here: http://i.imgur.com/O0gZn.png
		>>> filter = df.createGaussianFilter(im.width, im.height, dia=400,highpass=True,grayscale=True)
		>>> img = im.applyDFTFilter(filter)
		
		Output img: http://i.imgur.com/9hX5J.png

		>>> filter = df.createGaussianFilter(im.width, im.height, dia=400,highpass=False,grayscale=True)
		>>> img = im.applyDFTFilter(filter)
	...
	def applyUnsharpMask(self, inimg, boost=1,dia=400,grayscale=False):
		...
		* *inimg* - image to create and use mask on.
		...
		**SEE ALSO**
		...
		tempGFilt = self.createGaussianFilter(inimg.width,inimg.height,dia=dia,grayscale=grayscale,highpass=False)
		lpIm = inimg.applyDFTFilter(tempGFilt)
