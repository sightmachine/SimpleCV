...
"""
This class encapsulates almost everything needed to train, test, and deploy
a multiclass support vector machine for an image classifier. Training data
should be stored in separate directories for each class. This class uses the
feature extractor base class to  convert images into a feature vector. The
...
class SVMClassifier(object):
    ...
    mSVMProperties = {
        ...
        'gamma': None,        # kernel param for poly/rbf/sigma - default 1/#samples
    }
    ...
    def __init__(self, featureExtractors, properties=None):
        ...
        if not ORANGE_ENABLED:
            logger.warning("The required orange machine learning \
    ...
    def _parameterizeKernel(self):
        ...
        k = self.mSVMProperties["SVMType"]
        self.mSVMPrototype.svm_type = self.mSVMType[k]
        k = self.mSVMProperties["KernelType"]
        self.mSVMPrototype.kernel_type = self.mKernelType[k]
    ...
    def __setstate__(self, mydict):
        ...
        or_enum = orange.EnumVariable("type", values=self.mClassNames)
        self.mOrangeDomain = orange.Domain(map(orange.FloatVariable, colNames),
                                           or_enum)
    ...
    def setFeatureExtractors(self, extractors):
        ...
        extractors must match the ones used to train the classifier.
        If the classifier is already trained then this method will require
        that you retrain the data.
        """
    ...
    def train(self, images, classNames, disp=None, subset=-1, savedata=None, verbose=True):
        ...
        for im, cn in zip(images, classNames):
            if isinstance(im, str):
                count += self._trainPath(im, cn, subset, disp, verbose)
            else:
                count += self._trainImageSet(im, cn, subset, disp, verbose)
        ...
        if len(self.mClassNames) > 2:
            proto, dataset = self.mSVMPrototype, self.mDataSetOrange
            crossValidator = orngTest.learnAndTestOnLearnData([proto], dataset)
            confusion = orngStat.confusionMatrices(crossValidator)[0]

        if verbose:
            print("Correct: {0}".format(good))
            print("Incorrect: {0}".format(bad))
    ...
    def test(self, images, classNames,
             disp=None, subset=-1, savedata=None, verbose=True):
        """
        Test the classifier.
        images paramater can take in a list of paths or a list of imagesets.
        images - the order of the paths or imagesets must be in the same order
        ...
        for im, cn in zip(images, classNames):
            if isinstance(im, str):
                dataset, cnt, crct = self._testPath(im, cn, dataset, subset, disp, verbose)
                count = count + cnt
                correct = correct + crct
            else:
                dataset, cnt, crct = self._testImageSet(im, cn, dataset, subset, disp, verbose)
                count += cnt
                correct += crct
        ...
        confusion = 0
        if len(self.mClassNames) > 2:
            crossValidator = orngTest.learnAndTestOnTestData([self.mSVMPrototype],
                                                             self.mDataSetOrange,
                                                             testData)
            confusion = orngStat.confusionMatrices(crossValidator)[0]
    ...
    def _testPath(self, path, className, dataset, subset, disp, verbose):
        ...
        for i in range(nfiles):
            ...
            if testClass == c:
                text = "Classified as {0}".format(c)
                self._WriteText(disp, img, text, Color.GREEN)
                correct = correct + 1
            else:
                text = "Mislassified as {0}".format(c)
