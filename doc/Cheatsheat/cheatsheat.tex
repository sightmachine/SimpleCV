...
\begin{tabular}{llll}
  ...
  for a template image &  step(vision.TemplateMatcher,rgb2gray(img),rgb2gray(T))  &  & img.findTemplate(template\_image, threshold, method)\\[0.3cm]
 \end{tabular} 
\begin{tabular}{llll}
  \hline
  Description & Matlab & OpenCV & SimpleCV \\ \hline \\[.1cm] 
 ...
 form the Mask Image & & & minsize=10, maxsize=0 )\\[0.3cm]
  ...
  Creates a gaussian filter & H = fspecial(`gaussian',hsize,sigma);  & & \\
  of 64x64 pixels, resizes  & imfilter(I,H) &&img.applyGaussianFilter(dia, highpass, grayscale)\\
